    ...
    for i,capa in enumerate(list_capas):
        if capa[2] == 1:
            if capa[1] in [tb_manzanas,tb_sitios_interes,tb_ejes_viales]:
                print "select * from {} where ubigeo in ({}) ".format(capa[0], temp_ubigeos)
                if capa[1] ==tb_sitios_interes:
                    x = arcpy.MakeQueryLayer_management(path_conexion, 'capa{}'.format(i),
                                                        "select * from {} where ubigeo in ({})    AND (CODIGO<91 AND CODIGO<>26) ".format(capa[0],
                                                                                                         temp_ubigeos))
                else:
                    x = arcpy.MakeQueryLayer_management(path_conexion, 'capa{}'.format(i),
                                                    "select * from {} where ubigeo in ({})  ".format(capa[0], temp_ubigeos))
...
#
            #else:
#
            #    arcpy.CopyFeatures_management(tb_frentes,frente_capa)
