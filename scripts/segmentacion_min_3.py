...
class Segmentacion:
    ...
    database="INEI_BDCARTOGRAFIA_GIS"
    user = "sde"
    ...
    def __init__(self,cant_viv_techo_gra_ciud=16, cant_viv_techo_peq_ciud=16,techo_segunda_pasada_gra_ciud=16,techo_segunda_pasada_peq_ciud=16,cant_zonas_x=10,data_x=[],uso_falso_cod=1 ,campos_x=["UBIGEO","ZONA"],cant_viv_eleg_segund_pasada=9,techo_espe=0,col_mz_com=0,col_zona=0):
    ...
    def ObtenerZonasPrueba(self):
        ...
        if self.uso_falso_cod==1:
            ...
            sql_query = """
                    ...
                    select top {} a.ubigeo,a.zona from sde.prueba_segm_zona a
                    inner join (select distinct ubigeo,zona from  sde.prueba_segm_manzana where isnull(falso_cod,0)<>0 ) b on a.ubigeo=b.ubigeo and a.zona=b.zona
            ...
            sql_query = """
                ...
                select top {} ubigeo,zona from sde.prueba_segm_zona
                where flag_proc=0
                order by 1,2
    ...
    def ObtenerInformacionPrueba(self):
        ...
        sql_query = """
                    select ubigeo,codccpp,zona,isnull(id_estrato,0) id_estrato from sde.prueba_segm_zona WHERE ubigeo+zona in ({})
                    order by 1,2,3
                    """.format(where_zonas)
        print sql_query
        cursor.execute(sql_query)
        ...
        sql_query = """
                    select ubigeo,codccpp,zona,manzana,cant_viv,isnull(falso_cod,0) falso_cod,0 flag_cond,cant_pob from sde.prueba_segm_manzana WHERE ubigeo+zona in ({})
                    order by 1,2,3,4
                    """.format(where_zonas)
        print sql_query
        cursor.execute(sql_query)
        ...
        list_fields = [['UBIGEO', 'TEXT'],['CODCCPP','TEXT'] ,['ZONA', 'TEXT'],['ID_ESTRATO', 'SHORT'] ]
    ...
    def CrearAEUS(self):
        ...
        arcpy.Statistics_analysis(self.tb_rutas, self.tb_aeus, [["CANT_VIV", "SUM"],["CANT_POB","SUM"]], ["UBIGEO", "ZONA",  "AEU","FLG_MZ","TECHO","TECHO_S_P","FALSO_COD","TECHO_ESPE"])
        ...
        arcpy.AddField_management(self.tb_aeus,'COLAS_ZONA','SHORT')


        list_aeus_partidos=["{}{}{}".format(x[0],x[1],x[2]) for x in arcpy.da.SearchCursor(self.tb_aeus,["UBIGEO","ZONA","AEU"],"FLG_MZ=1")]


        with arcpy.da.UpdateCursor(self.tb_aeus, ["UBIGEO", "ZONA", "AEU", "CANT_VIV"]) as cursor:
            for x in cursor:
    ...
    def segunda_pasada_especial(self):
...
#
            ...
            aeus_eleg_temp = aeus_eleg[:]


            if int(len(aeus_eleg))>0:
                while aeus_eleg_temp>=0:
                    aeus_eleg_temp
...
#
    ...
    def uniendo_colas_por_zona(self):
        arcpy.env.overwriteOutput = True

        arcpy.CalculateField_management(self.tb_aeus,'COLAS_ZONA','1')
        lista_zonas = [(x[0], x[1], x[2]) for x in arcpy.da.SearchCursor(self.tb_zonas, ["UBIGEO", "ZONA", "ID_ESTRATO"], self.where_expression)]
        lista_rutas = []


        for zona in lista_zonas:
            where_expression1 = "UBIGEO='{}' and ZONA='{}' and FLG_MZ=0 and CANT_VIV<=10 ".format(str(zona[0]), str(zona[1]))
            numero_aeu = 1
            cant_vivi_agrupadas = 0
            ...
            list_aeus = [[x[0], x[1]] for x in arcpy.da.SearchCursor(self.tb_aeus, ["AEU", "CANT_VIV"],  where_expression1)]

            list_aeus_temp=list_aeus[:]
            ...
            #print list_aeus_temp
            conjuntos_conexos=[]
            conjunto_conexo=[]

            if len(list_aeus_temp)>0:
                i=1
                cant_vivi_agrupadas = 0
                while len(list_aeus_temp)>0:
                    el=list_aeus_temp[0]
                    aeu = el[0]
                    cant_viv = int(el[1])
                    cant_vivi_agrupadas = cant_vivi_agrupadas + cant_viv
                    ...
                    if cant_vivi_agrupadas <= self.cant_viv_techo:

                        aeus_renumerados[aeu]=aeu_temp
                        list_aeus_temp.remove(el)
            ...
            #print aeus_renumerados
            #print conjuntos_conexos

            with arcpy.da.UpdateCursor(self.tb_aeus,["UBIGEO", "ZONA", "AEU", "CANT_VIV"], where_expression1) as cursor:
                for x in cursor:
                    xaeu=int(x[2])
                    x[2]=aeus_renumerados[xaeu]
                    cursor.updateRow(x)



    def uniendo_colas_por_seccion(self):
        arcpy.env.overwriteOutput = True

        arcpy.CalculateField_management(self.tb_aeus, 'COLAS_ZONA', '2')


        #lista_zonas = [(x[0], x[1], x[2]) for x in
        #               arcpy.da.SearchCursor(self.tb_aeus, ["UBIGEO", "ZONA", "SECCION"], self.where_expression)]



        lista_secciones = set((x[0], x[1], x[2]) for x in
                       arcpy.da.SearchCursor(self.tb_aeus, ["UBIGEO", "ZONA", "SECCION"], self.where_expression))

        lista_rutas = []

        for seccion in lista_secciones:
            where_expression1 = "UBIGEO='{}' and ZONA='{}' and SECCION={} and FLG_MZ=0 and CANT_VIV<=9 ".format(str(seccion[0]),
                                                                                                  str(seccion[1]),str(seccion[2]))
            ...
            list_aeus = [[x[0], x[1]] for x in
                         arcpy.da.SearchCursor(self.tb_aeus, ["AEU", "CANT_VIV"], where_expression1)]
            ...
            with arcpy.da.UpdateCursor(self.tb_aeus, ["UBIGEO", "ZONA", "AEU", "CANT_VIV"],
                                       where_expression1) as cursor:
                for x in cursor:
                    xaeu = int(x[2])
                    x[2] = aeus_renumerados[xaeu]
                    cursor.updateRow(x)
    ...
    def InsertarRegistros(self):
        ...
        if arcpy.Exists("{}.sde".format(self.database)) == False:
        ...
        path_conexion2 = "Database Connections/{}.sde".format(self.database)
        #segm_ruta_prueba = path_conexion2 + "/{}.SDE.PRUEBA_SEGM_RUTA".format(self.database)
        segm_aeu_prueba = path_conexion2 + "/{}.SDE.PRUEBA_SEGM_AEU".format(self.database)
        ...
        for x in arcpy.da.SearchCursor(self.tb_zonas,["UBIGEO","ZONA","ID_ESTRATO"]):
            if int(x[2])==1:
                data.append([x[0],x[1],self.cant_viv_techo_gra_ciud,self.techo_segunda_pasada_gra_ciud,self.uso_falso_cod,self.techo_espe_real,self.col_zona])

            else:
                data.append([x[0], x[1], self.cant_viv_techo_peq_ciud, self.techo_segunda_pasada_peq_ciud,self.uso_falso_cod,self.techo_espe_real,self.col_zona])

        where_final=expresiones_consulta_arcpy.Expresion_2(data, [['UBIGEO', 'TEXT'], ['ZONA', 'TEXT'], ['TECHO', 'SHORT'], ['TECHO_S_P', 'SHORT'], ['FALSO_COD', 'SHORT'],['TECHO_ESPE','SHORT'],['COL_ZONA','SHORT']])
    ...
    def Segmentar(self):
        ...
        if (len(self.data)>0):
            ...
            if self.col_zona==1:
                self.uniendo_colas_por_zona()
                print "uniendo colas"
            if self.col_zona==2:
                self.uniendo_colas_por_seccion()
                print "uniendo colas por seccion"
...
crear_carpetas()
s=Segmentacion()
s.ResetearZonasPrueba()

for x in range(90):
   s = Segmentacion(cant_viv_techo_gra_ciud=16, cant_viv_techo_peq_ciud=16, techo_segunda_pasada_gra_ciud=16,
                     techo_segunda_pasada_peq_ciud=16, cant_zonas_x=100, data_x=data, uso_falso_cod=0, techo_espe=0,cant_viv_eleg_segund_pasada=0,col_zona=0)
   s.Segmentar()
