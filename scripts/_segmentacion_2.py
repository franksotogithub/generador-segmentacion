...
def crear_frentes_por_escala():
    list_frentes = [[x[0], x[1], x[2], int(x[3])] for x in
                    arcpy.da.SearchCursor(tb_frentes, ['UBIGEO', 'ZONA', 'MANZANA', 'FRENTE_ORD'])]
    puntos = arcpy.FeatureToPoint_management(tb_frentes, 'in_memory/puntos', "INSIDE")


    puntos_buffer = arcpy.Buffer_analysis(puntos, 'in_memory/buffer_puntos', '1 meters')
...
def CrearPuntosDeCorte():
    ...
    for x in list(set([x[0] for x in arcpy.da.SearchCursor(viviendas_frente_mfl, ["LLAVE_AEU"])])):
        ...
        if VivMaxAeu_frente[0] in VivMax:
            ...
            point = arcpy.CopyFeatures_management(VivMaxAeu_frente[6], u"in_memory/viv{}".format(VivMaxAeu_frente[0]))
            arcpy.AddField_management(point, "LLAVE_AEU", "TEXT")
            arcpy.AddField_management(point, "VIV_MAX", "TEXT")
            with arcpy.da.UpdateCursor(point, ["LLAVE_AEU","VIV_MAX"]) as cursorUC:
                for row in cursorUC:
...
def insertar_registros(data):
    ...
    for el in data:
        ...
        zona = el[1]
        sql_query = """
                DELETE GEODB_CPV_SEGM.SDE.SEGM_RUTA where ubigeo='{ubigeo}' and zona='{zona}'
                DELETE  GEODB_CPV_SEGM.SDE.SEGM_AEU where ubigeo='{ubigeo}' and zona='{zona}'
                delete GEODB_CPV_SEGM.SDE.SEGM_SECCION where ubigeo='{ubigeo}' and zona='{zona}'

                delete GEODB_CPV_SEGM.SDE.SEGM_SUBZONA  where ubigeo='{ubigeo}' and zona='{zona}'
                delete GEODB_CPV_SEGM.SDE.SEGM_RUTAS_LINEAS  where ubigeo='{ubigeo}' and zona='{zona}'
                delete GEODB_CPV_SEGM.SDE.SEGM_RUTAS_LINEAS_MULTIFAMILIAR  where ubigeo='{ubigeo}' and zona='{zona}'
...
def actualizar_capas():
    ...
    arcpy.AddField_management(tb_subzonas,'CANT_PAG','SHORT')
    arcpy.AddField_management(tb_subzonas, 'COD_CROQ', 'TEXT')
    arcpy.AddField_management(tb_subzonas, 'RUTA_CROQ', 'TEXT')
    arcpy.AddField_management(tb_subzonas, 'RUTA_WEB', 'TEXT')

    with arcpy.da.UpdateCursor(tb_subzonas, ['UBIGEO','ZONA','SUBZONA','CANT_PAG','COD_CROQ','RUTA_CROQ','RUTA_WEB']) as cursor:
...
def ExportarSegmTab(data=[],campos=["UBIGEO","ZONA"]):
    if len(data)>0:
        ...
        print 'Actualizar Cantidad paginas'
