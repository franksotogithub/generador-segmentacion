...
#path_urbano_croquis_listado = path_out + "\\croquis-listado\\urbano"
...
tb_rutas_lineas_temp= path_ini + "/tb_rutas_lineas_temp.shp"
...
#path_resultados=path_ini +"\\resultados"
#path_resultados_zona=path_resultados +"\\{}{}".format(ubigeox,zonax)

path_resultados="\\\\cpc_seg23\\file_resultados_proceso_segm"
path_resultados_zona=path_resultados +"\\{}{}".format(ubigeox,zonax)
...
def importar_tablas_trabajo(data, campos):
    ...
    list_capas = [
        ["{}.sde.TB_MANZANA".format(db), tb_manzanas, 1],
        ["{}.sde.TB_SITIO_INTERES".format(db), tb_sitios_interes, 1],
        ["{}.sde.TB_PUNTO_INICIO".format(db), tb_puntos_inicio, 1],
        ["{}.sde.VW_ZONA_CENSAL".format(db), tb_zonas, 1],
        ["{}.sde.TB_EJE_VIAL".format(db), tb_ejes_viales, 1],
        ["{}.sde.VW_VIVIENDAS_U".format(db), tb_viviendas, 1],
        ["{}.sde.VW_FRENTES".format(db), tb_frentes, 1],
    ...
    for i,capa in enumerate(list_capas):
        ...
        if capa[1] in [tb_viviendas,tb_manzanas,tb_frentes]:
...
def crear_frentes_por_escala():
    ...
    list_zonas=[[x[0], x[1], int(x[2])] for x in
                    arcpy.da.SearchCursor(tb_zonas, ['UBIGEO', 'ZONA','FLAG_SELEC'])]
    list_frentes = [[x[0], x[1], x[2], int(x[3]),int(x[4]),int(x[5])] for x in
                    arcpy.da.SearchCursor(tb_frentes, ['UBIGEO', 'ZONA', 'MANZANA', 'FRENTE_ORD','FALSO_COD','FLAG_SELEC'])]
    ...
    for i, frente_capa in enumerate(list_frentes_capas):
        puntos_buffer = arcpy.Buffer_analysis(puntos, 'in_memory/buffer_puntos{}'.format(i),
                                              '{} meters'.format(metros_ini + i))
        ...
        for x in list_frentes:
            ...
            buffer_lyr = arcpy.SelectLayerByAttribute_management(buffer_lyr, "NEW_SELECTION",
                                                                 u"UBIGEO='{}' AND ZONA='{}' AND MANZANA='{}' AND FRENTE_ORD={}".format(
                                                                     x[0], x[1], x[2], x[3]))
            clip = arcpy.Clip_analysis(frente_lyr, buffer_lyr,
                                       '{}\\clip{}{}{}{}.shp'.format(path_clips_frentes, x[0], x[1], x[4], x[3]))
...
def CrearRutasPreparacion(manzanas):
    ...
    #if arcpy.Exists_management(tb_puntos_corte):
...
def CrearLineasMultifamiliar():
    ...
    for m in arcpy.da.SearchCursor(fc_Edificios_mfl,["FID", "UBIGEO", "ZONA", "AEU", "ID_REG_OR", "SHAPE@X","SHAPE@Y", "MANZANA","TIPO"]):
        ...
        if round((inicio_fin[0]- inicio_fin[2]),8)!=0.0:
...
def seleccionar_muestra_aeu_calidad():
    arcpy.AddField_management(tb_aeus, "FLAG_CALID", "SHORT")
    zonas = list(set((x[0], x[1]) for x in arcpy.da.SearchCursor(tb_zonas, ['UBIGEO', 'ZONA'])))

    for zona in zonas:
        ubigeox=zona[0]
        zonax = zona[1]

        totalEmp = len(  [x[0]  for x in  arcpy.da.SearchCursor(tb_aeus, ['UBIGEO','ZONA', 'AEU']," UBIGEO='{}' AND ZONA='{}' ".format(ubigeox,zonax) )])
        ...
        if  totalEmp == 1:
            tam_muestra = 1
        elif totalEmp <= 25 and totalEmp > 1:
            tam_muestra = 2

        elif totalEmp >= 26 and totalEmp <= 150:
            tam_muestra = 8
        elif totalEmp >= 151 and totalEmp <= 280:
            tam_muestra = 13
        elif totalEmp >= 281 and totalEmp <= 500:
            tam_muestra = 20
        elif totalEmp >= 501:
            tam_muestra = 32

        if tam_muestra > 1:
            intervalo = totalEmp / tam_muestra
            inic = random.randint(1, int(intervalo))
            aleat = []
            listaAlea = inic;


            for n in range(tam_muestra):
                aleat.append(listaAlea)
                listaAlea = listaAlea + intervalo
        ...
        with arcpy.da.UpdateCursor(tb_aeus,['AEU','FLAG_CALID']," UBIGEO='{}' AND ZONA='{}' ".format(ubigeox,zonax) ) as  cursor:
            for x in cursor:
                if int(x[0]) in aleat:
                    x[1]=1
                    cursor.updateRow(x)
...
def seleccionar_muestra_seccion_legajo():
    ...
    for zona in zonas:
        ...
        totalsecc= len(  [x[2]  for x in  arcpy.da.SearchCursor(tb_secciones, ['UBIGEO','ZONA', 'SECCION']," UBIGEO='{}' AND ZONA='{}' ".format(ubigeox,zonax))] )
        ...
        if totalsecc == 1:
            ...
            tam_muestra = 2
        elif totalsecc >= 3 and totalsecc <= 90:
            tam_muestra = 3
        elif totalsecc >= 91 and totalsecc <= 280:
            tam_muestra = 13
        elif totalsecc >= 281 and totalsecc <= 500:
            tam_muestra = 20
        elif totalsecc >= 501 and totalsecc <= 1200:
            tam_muestra = 32


        if tam_muestra > 1:
            intervalo = totalsecc / tam_muestra
            inic = random.randint(1, int(intervalo))
            ...
            aleat=[1]
        with arcpy.da.UpdateCursor(tb_secciones,['SECCION','FLAG_CALID']," UBIGEO='{}' AND ZONA='{}' ".format(ubigeox,zonax) ) as  cursor:
            for x in cursor:
                if int(x[0]) in aleat:
                    x[1]=1
                    cursor.updateRow(x)
...
def preparar_registros():
    ...
    for el in list_capas:
        ...
        if (el==tb_viviendas_ordenadas):
            ...
            list_campos_validos = ['FID', 'Shape', 'UBIGEO', 'CODCCPP', 'ZONA', 'MANZANA', 'ID_REG_OR', 'FRENTE_ORD','AEU','ID_VIV','OR_VIV_AEU','P19A',
                                   'P20','P21','P22_A','P23','P24','P25','P26','P27_A','P28','P29','P29_A','P29_P','JEFE_HOGAR','ID_REG','ID_REG_PAD','CANT_POB']
            delete_fields = []
            for el in fields:
...
def copiar_registros():
    ...
    if arcpy.Exists(tb_rutas_lineas_multifamiliar):
        list_capas_ini.append(tb_rutas_lineas_multifamiliar)

    for i, el in enumerate(list_capas_ini):
        dir = os.path.split(el)
        dir_copia = os.path.join(dir[0], "final_{}".format(dir[1]))
        list_capas_export.append(dir_copia)

    list_capas_export.extend([tb_frentes_1,tb_frentes_2,tb_frentes_3,final_sitios_interes,tb_rutas_puntos])


    for i,el in enumerate(list_capas_export):
        nom_capa=os.path.split(el)[1]
        path_resultado_capa=os.path.join(path_resultados_zona,'{}'.format(nom_capa))
        formato = nom_capa.split(".")[1]

        if formato == 'shp':
            arcpy.CopyFeatures_management(el, path_resultado_capa)
        else:
            arcpy.CopyRows_management(el, path_resultado_capa)
...
def ExportarCroquisUrbanoAEU(where_expression=''):
    ...
    with arcpy.da.UpdateCursor(final_aeu, ["UBIGEO", "ZONA","CODCCPP" ,"SECCION", "AEU", "CANT_VIV",'CANT_PAG','COD_CROQ' ,'RUTA_CROQ',
        ...
        for row in cursor:
            ...
            if ((df.scale > 1300.0) and arcpy.Exists(tb_rutas_lineas_multifamiliar)):
...
def ExportarCroquisUrbanoSeccion(where_expression=''):
    ...
    with arcpy.da.UpdateCursor(final_seccion,
        ...
        for row in cursor:
            ...
            #list_viv_aeu_min=[ [v[0],v[1],v[2],v[3],v[4],v[5],v[6],v[7],v[8],v[9],v[10],v[11],v[12],v[13]] for v in arcpy.da.SearchCursor(viviendas_aeu_min, ['MANZANA', 'P29', 'P20', 'P21', 'P22_A', 'P23', 'P25', 'P26', 'P27_A', 'P32','P19A', 'FRENTE_ORD', 'P29_A', 'P29_P'])]
            list_viv_aeu_min = [[v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8], v[9], v[10], v[11], v[12], v[13]]
                                for v in arcpy.da.SearchCursor(viviendas_aeu_min,
                                                               ['MANZANA', 'P29', 'P20', 'P21', 'P22_A', 'P23', 'P25',
                                                                'P26', 'P27_A', 'JEFE_HOGAR', 'P19A', 'FRENTE_ORD', 'P29_A',
                                                                'P29_P'])]


            #list_viv_aeu_max = [[v[0], v[1], v[2], v[3], v[4], v[5], v[6],v[7], v[8], v[9], v[10], v[11], v[12], v[13]] for v in arcpy.da.SearchCursor(viviendas_aeu_max,['MANZANA', 'P29', 'P20', 'P21', 'P22_A', 'P23', 'P25', 'P26','P27_A', 'P32', 'P19A', 'FRENTE_ORD', 'P29_A', 'P29_P'])]

            list_viv_aeu_max = [[v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8], v[9], v[10], v[11], v[12], v[13]]
                                for v in arcpy.da.SearchCursor(viviendas_aeu_max,
                                                               ['MANZANA', 'P29', 'P20', 'P21', 'P22_A', 'P23', 'P25',
                                                                'P26', 'P27_A', 'JEFE_HOGAR', 'P19A', 'FRENTE_ORD', 'P29_A',
                                                                'P29_P'])]
            primera_puerta=list_viv_aeu_min[0]
...
def SegmentacionTabular(data,campos=["UBIGEO","ZONA"]):
    ...
    print datetime.today()
    conx.actualizar_cant_viv_mzs(data)
    ...
    print datetime.today()
    seleccionar_muestra_aeu_calidad()
    print "seleccionar_muestra_aeu_calidad"
    print datetime.today()
    seleccionar_muestra_seccion_legajo()
    print datetime.today()
    print "seleccionar_muestra_seccion_legajo"
    print datetime.today()
